...
public class Facade
{
    ...
    //HISTORIA DE USUARIO DEFENSA
    public string GetPlayerWinning(Turn turn, Player player)
    {
        //Me aseguro que el juegador no sea nulo
        if (player == null)
        {
            return $"{player.DisplayName} no está jugando";
        }
        //Me aseguro que turn tampoco sea null
        if (turn == null)
        {
            return $"No es el turno de {player.DisplayName}";
        }
        foreach (Pokemon pokemon in player.AvailablePokemons)
        {
            if (turn.CurrentPlayer == player)
            {
                return player.TurnChanged();
        ...
        return UserInterface.ReturnMessageWhoIsWinningWhenFinishedTurn(player);
